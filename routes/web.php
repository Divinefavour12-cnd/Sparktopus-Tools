...
Route::group(
    ['middleware' => ['FrontTheme', 'doCache', 'auth', '2fa']],
    function () {
        ...
        Route::post('user/notifications/mark-as-read/{id}', [UserController::class, 'markAsRead'])->name('user.notifications.read');
        Route::get('user/notifications/mark-all-read', [UserController::class, 'markAllAsRead'])->name('user.notifications.readAll');
    }
);
...
Route::group(
    ...
    function () {
        ...
        Route::get('/tools', [HomeController::class, 'allTools'])->name('front.tools');
        // Static pages (no /page)
Route::get('{slug}', [PageController::class, 'show'])
    ->name('pages.show')
    ->where('slug', 'about|contact|cookie|refund');
        // Blog
      // Updates (Blog renamed)
Route::get('updates', [BlogController::class, 'index'])->name('updates.index');
Route::get('updates/{slug}', [BlogController::class, 'show'])->name('updates.show');
