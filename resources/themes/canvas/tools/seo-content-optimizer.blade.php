<x-application-tools-wrapper>
    <x-tool-wrapper :tool="$tool">
        ...
        <x-form method="post" :route="route('tool.handle', $tool->slug)">
            ...
            @if(isset($results['remaining']))
                <div class="alert alert-info mt-3">
                    You have <strong>{{ $results['remaining'] }}</strong> uses remaining on your
                    <strong>{{ $results['plan'] }}</strong> plan.
                </div>
            @endif
            ...
            <div class="row mb-4">
                <div class="col-md-12 mt-2 mb-3">
                    <div class="form-group">
                        <x-textarea-input type="text" name="string" class="form-control" rows="10"
                            :placeholder="__('Paste your content to optimize for SEO...')" id="textarea" required autofocus contenteditable="true">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="box-shadow tabbar mb-3 custom-textarea">
                                <x-textarea-input id="seo-result" class="form-control transparent" rows="12">
                                    {{ $results['converted_text'] }}
                                </x-textarea-input>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <x-copy-target target="seo-result" />
                                    <x-download-form-button type="button"
                                        onclick="ArtisanApp.downloadAsTxt('#seo-result', {filename: '{{ $tool->slug . '.txt' }}'})"
                                        :tooltip="__('tools.saveAsTxt')" />
                                    <x-print-button
                                        onclick="ArtisanApp.printResult(document.querySelector('#seo-result'), {title: '{{ $tool->name }}'})"
