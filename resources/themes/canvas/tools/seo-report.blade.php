            ...
            <div class="row custom-textarea-wrapper">
                <div class="col-md-12 mb-4">
                    <div class="custom-textarea p-4 shadow-sm border rounded bg-white">
                        <x-input-label class="h4 fw-bold mb-3">@lang('tools.enterWebsiteUrl')</x-input-label>
                        <div class="input-group input-group-lg shadow-sm rounded-pill overflow-hidden border">
                            <span class="input-group-text bg-white border-0 ps-4"><i class="bi bi-link-45deg fs-4 text-muted"></i></span>
                            <x-text-input class="form-control border-0 shadow-none py-3" name="url" id="url" type="url" required
                                value="{{ $results['url'] ?? old('url') }}" :placeholder="__('tools.enterOrPasteUrl')" />
                            <x-button type="submit" class="btn btn-primary px-5 fw-bold">
                                @lang('tools.generateReport')
                            </x-button>
                        </div>
                        <x-input-error :messages="$errors->get('url')" class="mt-2 ms-3" />
                        
                        <div class="mt-4 text-muted small ms-2">
                            <i class="bi bi-info-circle me-1"></i> @lang('Enter the clinical URL of any website to get a comprehensive SEO audit.')
                        </div>
                    </div>
                </div>
            </div>
