...
@if (!empty($advertisement) && ((Auth::check() && Auth::user()->is_ads_allowed) || !Auth::check()))
    <div class="row">
                ...
                <div class="ad-card-outer">
                    <div class="ad-card-wrap bg-info-light">
                        @if (($advertisement->type == 2 || $advertisement->type == 1) && !empty($advertisement->options['image']))
                            <a class="ad-card-img" href="{{ $advertisement->options['target_url'] }}" tabindex="-1">
                                <div>
                        ...
                        @if ($advertisement->type == 1)
                            <a href="{{ $advertisement->options['target_url'] }}" target="_blank"
                                rel="nofollow noopener noreferrer" tabindex="-1">
                                <div class="ad-card-content">
                                    <div class="ad-card-title">{!! $advertisement->title !!}</div>
                                    <div class="ad-card-link">
                                        {{ extractHostname($advertisement->options['target_url']) }}
                                    </div>
                        ...
                        @endif
                        <a href="{{ route('plans.list') }}" class="ad-remove-button bg-info-light" tabindex="-1">Enjoy Sparktopus Tools Ad-Free. [Upgrade now]</a>
