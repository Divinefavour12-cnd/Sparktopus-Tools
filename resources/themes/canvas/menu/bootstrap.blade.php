...
@auth
    @if (!isset($innerLoop))
        @php
            $currentLevel = auth()->user()->planLevel();
        
            
            // Define upgrade paths
            $upgradePaths = [
                'free' => ['label' => 'Classic', 'color' => 'info'],
                'classic' => ['label' => 'Plus', 'color' => 'warning'],
                'plus' => ['label' => 'Pro', 'color' => 'success'],
                'pro' => null // Already at top tier
            ];
            
            $nextUpgrade = $upgradePaths[$currentLevel] ?? null;
        @endphp
...
@endif
        ...
        @if ($currentLevel !== 'free')
            <li class="nav-item">
                <span class="nav-link text-primary fw-bold">
                    {{ ucfirst($currentLevel) }} {{ __('Subscription') }}
                </span>
            </li>
        @endif
        ...
        @if ($nextUpgrade)
            <li class="nav-item">
                <a class="nav-link text-warning fw-bold" href="{{ route('plans.list') }}">
                    {{ __('Upgrade to') }} {{ $nextUpgrade['label'] }}
                </a>
            </li>
        @else
            {{-- Show Upgrade CTA if on free --}}
            @if ($currentLevel === 'free')
                <li class="nav-item">
                    <a class="nav-link text-warning fw-bold" href="{{ route('plans.list') }}">
                        {{ __('Upgrade') }}
