...
class Tool extends BaseModel implements TranslatableContract, HasMedia, Viewable, Sitemapable
{
     ...
     * @var array
     */
   protected $fillable = ['slug', 'display', 'status', 'settings', 'properties', 'icon_class', 'icon_type', 'is_home', 'required_plan'];
...
{
    return Auth::check() ? !Auth::user()->shouldSeeAds() : false;
}
    /**
    ...
    /**
     * Determine if the tool is an AI-integrated tool.
     *
     * @return bool
     */
    public function isAiTool()
    {
        $aiTools = [
            'App\Tools\AiHumanizer',
            'App\Tools\RewriteArticle',
            'App\Tools\TextSummarizer',
            'App\Tools\GrammarChecker',
            'App\Tools\BlogTitleGenerator',
        ];

        return in_array($this->class_name, $aiTools);
    }
}
