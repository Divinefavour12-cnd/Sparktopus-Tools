...
class ToolController extends Controller
{
    ...
    public function index(Request $request, $tool)
    {
        ...
        // Check plan access for authenticated users
        if (Auth::check()) {
            $requiredLevel = $tool->required_plan ?? 'free';
            
            if (!Auth::user()->canAccessTool($requiredLevel)) {
                return redirect()->route('plans.list')
                    ->with('error', 'This tool requires a ' . ucfirst($requiredLevel) . ' plan or higher. Please upgrade to continue.');
            }
        } else {
            // Guest users can only access free tools
            if (($tool->required_plan ?? 'free') !== 'free') {
                return redirect()->route('login')
                    ->with('error', 'Please login or upgrade to access this tool.');
            }
        }
    ...
    public function handle(Request $request, $tool)
    {
        try {
            ...
            if (Auth::check()) {
                ...
                if (!Auth::user()->canAccessTool($requiredLevel)) {
                    return redirect()->back()->withErrors('This tool requires a ' . ucfirst($requiredLevel) . ' plan or higher.');
                }
            }
