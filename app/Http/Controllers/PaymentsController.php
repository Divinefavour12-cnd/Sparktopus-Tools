...
class PaymentsController extends Controller
{
    ...
    public function success(Request $request, $id)
    {
        ...
        if ($user) {
            $user->notify(new AppNotification(
                ...
                __('Your :plan plan is now active.', ['plan' => $transaction->plan->name]),
                'success',
                route('user.plan')
            ));
            ...
            $user->notify(new AppNotification(
                ...
                __('We have successfully processed your payment of :amount :currency.', [
    ...
    public function webhookListener(Request $request)
    {
        ...
        if ($transaction) {
            ...
            $user = $transactionnew->user;
            if ($user) {
                $user->notify(new AppNotification(
                    __('Subscription Renewed'),
                    __('Your :plan plan has been successfully renewed.', ['plan' => $transactionnew->plan->name]),
                    'success',
                    route('user.plan')
                ));
            }
